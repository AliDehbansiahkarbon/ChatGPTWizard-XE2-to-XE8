...
uses
  ...
  UChatGPTSetting, UConsts, System.StrUtils;
...
type
  ...
  public
    constructor Create(AHandle: HWND; AApiKey, AModel, APrompt, AUrl: string; AMaxToken, ATemperature: Integer;
                       AAnimated: Boolean; ATimeOut: Integer; AIsOffline: Boolean);
  ...
  public
    ...
    function QueryOllamaIndy(const AModel: string; const APrompt: string): string;
  end;
...
function TOpenAIAPI.QueryOllamaIndy(const AModel: string; const APrompt: string): string;
var
...
begin
  ...
  try
    ...
    try
      LvResponseContent.Text := LvHttpClient.Post(FUrl, LvParamStream);
...
//        Result := 'No answer!';
      ...
      if not LvResponseContent.Text.IsEmpty then
        Result := AdjustLineBreaks(UTF8ToString(ExtractOllamaAnswer(LvResponseContent)))
      else
...
begin
  ...
  try
    try
      ...
      begin
        if FIsOffline then
          LvResult := LvAPI.QueryOllamaIndy(FModel, FPrompt).Trim
        else
